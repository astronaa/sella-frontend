...
export const schema = schemaUpdate
  .merge(schemaUploadImages)
  .merge(z.object({ galleryImagesUrls: z.array(z.string()) }));

export type SchemaType = z.infer<typeof schema>;

export async function manageProduct(productId: ProductId, values: SchemaType) {
  const { galleryImagesUrls, galleryImages, previewImage, ...payload } =
    schema.parse({
      ...values,
