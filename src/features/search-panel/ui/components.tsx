...
import { StoreCard, StoreLink, storeQueries } from "~/entities/store";
...
export function Root({ children }: PropsWithChildren) {
  ...
  return <SearchPanelProvider value={value}>{children}</SearchPanelProvider>;
}
...
export function Content({
  ...
  const productItems = products?.items.slice(0, resultsToShow);
  const storeItems = stores?.items.slice(0, 2);
  const productTotal = products?.total;
  const storeTotal = stores?.total;
  const showMore = !!productTotal && productTotal > 2;
  ...
  return (
    <div
      {...props}
      className={cn(
        "flex flex-col gap-[3rem] w-full max-w-content mx-auto mb-[3rem]",
        className
      )}
    >
      {/* Show stores first if any found */}
        ...
        <div className="flex flex-col gap-[1rem] w-full">
          <div className="flex gap-[1rem] justify-between items-center w-full">
            <Heading size="sm" className="max-md:text-[1.25rem]">
              ...
              {storeTotal !== undefined && (
                <span className="text-black-40">{storeTotal}</span>
              )}
            </Heading>
          </div>
          ...
          <div className="flex w-full gap-[1rem] justify-between">
            {storeItems.map((store) => (
              <StoreLink key={store.url} store={store} className="h-full">
                <StoreCard.Composed store={store} className="h-full" />
              </StoreLink>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col gap-[1rem] w-full">
        <div className="flex gap-[1rem] justify-between items-center w-full">
          <Heading size="sm" className="max-md:text-[1.25rem]">
            ...
            {productTotal !== undefined && (
              <span className="text-black-40">{productTotal}</span>
            )}
          </Heading>
        </div>
          ...
          storeItems.length == 0 && (
            <div className="flex flex-col items-center gap-[0.75rem]">
              <Heading size="sm" className="text-[1.5rem] text-black-60">
                ...
                {category ? `in ${category.name}` : null}
              </Heading>
            </div>
          )}
          ...
          <>
            {isLoadingProducts &&
              Array(resultsToShow)
                .fill(productMock)
                .map((p, index) => (
                  <Skeleton key={index} loading asChild>
                    <MobileProductCard product={p} />
                  </Skeleton>
                ))}

            {productItems?.map((p, index) => (
              <MobileProductCard key={index} product={p} />
            ))}
          </>
        ) : (
          <Scrollable.Root scrollOptions={{ dragFree: false }}>
        ...
        {showMore && (
          <Button asChild colorPalette="gray" size="xl">
    ...
    >
      <ProductLink product={product}>
        ...
        <ProductCard.Content className="gap-[0.5rem] px-0 max-w-full">
          <ProductCard.Title />
          <ProductCard.Description className="whitespace-normal line-clamp-2" />
          <ProductCard.Price />
        </ProductCard.Content>
      </ProductLink>
    </ProductCard.Root>
