...
export interface TextAreaProps
  extends InputVariantProps,
    Omit<HTMLArkProps<"textarea">, "size"> {
  error?: boolean;
}
...
const styles = tv(
  {
    base: [
      "text-white outline-none transition leading-[normal] placeholder-black-40 bg-transparent",
    ],
    defaultVariants: {
      variant: "filled",
      size: "md",
    ...
    variants: {
      ...
      size: {
        ...
        "2xl":
          "rounded-[0.625rem] min-h-[6.25rem] min-w-[4.75rem] px-[1.625rem] text-[2rem]",
      },
    },
  },
  { twMerge: true }
...
export function InputGroup({ className, ...props }: HTMLArkProps<"div">) {
  return <ark.div {...props} className={cn("relative", className)} />;
}
